...
1. **録画バックアップ**: デモ前日に、十分な履歴がある環境で 3 スキル全ての正常実行を画面録画しておく。最悪の場合、録画を再生してデモとする
2. **当日メモ**: mixed-scope の説明、`daily-report` の mode 差分、`skill-miner` の 3 区分を短く言い直せるメモを手元に用意する

#### 本番中の判断基準

- **source 0 本 / 履歴密度不足**: aggregate.py が空結果を返した場合、graceful degrade の空日報をそのまま説明し、必要なら録画バックアップへ切り替える
- **aggregate.py がエラー**: 「裏でデータ収集中にエラーが出ましたが、DayTrace はエラー時も止まらず簡易版を返します」と言い、録画バックアップへ切り替える
- **skill-miner が 0 件 proposal**: 「履歴が少ない環境ではこうなります」と説明し、提案が出るケースは録画バックアップで補う
- **全スキル失敗**: 録画バックアップを再生
...
- 録画バックアップへの切り替えを隠さない。「事前に撮った実行です」と正直に言う
