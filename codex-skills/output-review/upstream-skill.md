...
DayTrace の出力品質を output-polish.md 基準でレビューし、P 番号プランの未実装項目を特定して SKILL.md に反映する。開発リポジトリ専用（配布プラグインの skill には含めない）。
...
リポジトリルートの `docs/output-polish.md` の §12（実装優先順）を読む。

対象 SKILL は **配布プラグイン側**のため、パスは `plugins/daytrace/skills/<skill名>/SKILL.md` とする（`references/p-plan-check.md` 表の `skills/...` は `plugins/daytrace/skills/...` として解釈する）。
...
1. P7: 見出し構造の §9 準拠徹底
   変更対象:
   - plugins/daytrace/skills/daily-report/SKILL.md → 共有用日報の見出しを §9-2 に合わせる
   - plugins/daytrace/skills/post-draft/SKILL.md → 見出し構造セクションを §9-3 と照合
   「P7 を適用して」と言うと diff-preview を出します。
```

## Phase 4: 実装（ユーザー指示後）

1. 対象 SKILL.md を Read する（`plugins/daytrace/skills/...`）
