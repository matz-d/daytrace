...
- `skill-miner`: SKILL.md → `skill_miner_prepare.py` で compressed candidate view を生成 → ユーザー選択 → `skill_miner_detail.py` で選択候補だけ detail 再取得 → Claude がドラフト生成
...
- Python スクリプトで `git log --numstat` をパース
- additions / deletions / changed files を JSON で返す
...
1. `skill_miner_prepare.py` で全セッションを packet 化・cluster 化し、compressed candidate view を生成
2. 分類 + 提案リスト出力（自動）
3. ユーザーに「どれをドラフト化するか」確認
4. `skill_miner_detail.py` で選択候補の `session_ref` だけ detail 再取得
5. 選択された候補のドラフトを生成
