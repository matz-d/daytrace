...
|--------|---------|----------------------|-----------|
| `git-history` | `workspace` | `git` | カレントリポジトリの Git コミット履歴 |
| `claude-history` | `all-day` | `ai_history` | `~/.claude/projects/**/*.jsonl` の会話ログ |
| `codex-history` | `all-day` | `ai_history` | `~/.codex/history.jsonl`, `~/.codex/sessions/` の会話ログ |
