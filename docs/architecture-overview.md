...
- SQLite store（`source_runs` / `observations` / `activities` / `patterns`、schema v3）
- projection adapters（`daily_report_projection.py`, `post_draft_projection.py`, `projection_adapters.py`）
...
- `docs/skill-catalog.md`: 5 skill の役割・入出力・使い分けガイド
