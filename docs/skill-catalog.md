...
| `skill-applier` | 固定化成果物（CLAUDE.md diff / skill scaffold / 設計案） | skill-miner の proposal JSON | suggested_kind による dispatch |
...
| `daytrace-session` | today (デフォルト) | 任意、補助フィルタ | mode 未指定は自分用 + 条件付き共有用 | 0 回（共有用機密境界確認・固定化確認・CLAUDE.md diff は例外） |
...
| `skill-applier` | CLAUDE.md diff preview / skill scaffold context / hook・agent 設計案 | Markdown |
...
| 5（条件付き） | `skill-applier` skill に委譲 | 候補選択後の固定化アクション実行 |
...
- `skill` / `hook` / `agent` の実際のコード生成・自動デプロイは行わない（設計案・scaffold の提示のみ）
...
skill-miner が返した proposal の candidate を `suggested_kind` に応じた固定化成果物に変換する。
`CLAUDE.md` だけは immediate apply path を持ち、`skill` / `hook` / `agent` は設計案・scaffold の提示のみ。
...
- `hook` / `agent`: 設計案（trigger / action / scope）を提示し、次セッションへ送る
...
- `skill` / `hook` / `agent` は即時生成しない（設計案・handoff の提示のみ）
...
| 固定化アクション | ready >= 1 & 候補選択あり | skill-applier に委譲 | 全候補を pending で記録 |
...
- 共有用日報の機密境界確認・ready proposal の固定化確認・CLAUDE.md diff preview では、例外的にユーザー確認を待つ
...
- `skill-applier` で特定の proposal を手動で固定化したい場合
...
抽出と固定化の責務が異なるため。`skill-miner` はパターンを発見して評価・分類する責務を持ち、`skill-applier` は選択された候補を成果物（CLAUDE.md ルール・skill scaffold・設計案）に変換する責務を持つ。分離することで、提案を見てから固定化を別セッションで行うワークフローも成立する。
