...
| `daytrace-session` | 上記 4 つ + 状態見出し付きセッションサマリ（§5-5 mixed-scope / 再構成元） | 日本語 Markdown + `~/.daytrace/output/<date>/` artifact |
...
**Phase 3（Post Draft）の起動条件**: `git + (claude or codex) 共起 sources success` または `total_groups >= 4`
