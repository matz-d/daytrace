...
| `daytrace-session` | 上記 4 つ + 状態見出し付きセッションサマリ（`docs/output-polish.md` §5-5: mixed-scope 注記・再構成元の要約） | 日本語 Markdown + `~/.daytrace/output/<date>/` artifact |
...
| Phase 3 | Post Draft（条件付き） | `post_draft_projection.py` |
| Phase 4 | Pattern Mining & Proposals & Fixation | `skill_miner_prepare.py` → (条件付き) `detail` → `judge` → `proposal` → `skill-applier` |
| Phase 5 | Session Summary | — |

**Phase 3（Post Draft）の起動条件**（Phase 1 の JSON を参照。正本は `plugins/daytrace/skills/daytrace-session/SKILL.md` Phase 3）:

- **いずれか**を満たすとき生成する:
  - `sources` において `git-history` と（`claude-history` または `codex-history`）が**いずれも `success`**（略称: **AI + Git 共起**）
...
| 投稿下書き | AI + Git 共起（各 source `success`）または `total_groups >= 4` | 生成 | スキップ |

### 7-3. ユーザー向け状態表示

`[DayTrace]` プレフィックスは使わない。見出し（例: `### ソース収集の結果`）と短い日本語で、スキップ理由も同様に示す。詳細は `plugins/daytrace/skills/daytrace-session/SKILL.md` と `docs/output-polish.md` §6。
...
- Phase 2 〜 4（日報・投稿下書き・パターン提案）の品質は各個別 skill の品質に依存する
