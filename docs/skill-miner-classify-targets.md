...
Phase 4（Pattern Mining）の **classification overlay**（`classification-prompt.md` に従った 1 候補 1 JSON）は、LLM 呼び出しコストとユーザー認知負荷を抑えるため、**全候補ではなく「曖昧な候補だけ」**に限定する。
...
セッション内のユーザー向け状態（例: `### パターン提案` の一行要約）に、**件数**だけでなく **絞り込み規準の一言**を載せてもよい（`[DayTrace]` は使わない）。

- 例: `classify 対象 2/6（境界: skill/agent・skill/CLAUDE.md、弱確信のみ）`

## 関連

- 手順の全体: `plugins/daytrace/skills/daytrace-session/SKILL.md` Phase 4（Pattern Mining）
- 配布用の同一文書: `plugins/daytrace/skills/skill-miner/references/classify-target-selection.md`
- overlay contract: `plugins/daytrace/skills/skill-miner/references/classification-prompt.md`
- 分類境界: `plugins/daytrace/skills/skill-miner/references/classification.md`
