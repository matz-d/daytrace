...
store path のデフォルト: `~/.daytrace/daytrace.sqlite3`（schema version: 2）
...
| カラム | 型 | 役割 |
|-------|-----|-----|
| `query_fingerprint` | TEXT | クエリ文脈の一意識別子（workspace + since + until + all_sessions + group_window + derivation_version） |
| `derivation_version` | TEXT | `activities-v1` |
...
`PRAGMA user_version` で確認できる。現行は `2`。
version 1 から 2 への自動マイグレーションは `bootstrap_store()` が担い、`confidence_categories_json` カラムを追加する。
...
- `plugins/daytrace/scripts/derived-layer-note.md`: derived layer の設計方針と versioning ルール
- `plugins/daytrace/scripts/store-run-context-note.md`: `source_runs` / `observations` の保存方針と fingerprint policy
- `TODO-AR4b-store-hardening.md`: store 堅牢化タスクの詳細
